...
function isDisabledForCurrentRole() {
	...
	$user = wp_get_current_user();
	$disabled_for = \fmb_engine\Facebook\FacebookRuntime()->getOption( 'do_not_track_user_roles' );
	...
	foreach ( (array) $user->roles as $role ) {
		...
		if ( in_array( $role, $disabled_for ) ) {
			...
			add_action( 'wp_head', function() {
				echo "<script type='application/javascript' id='fmb-engine-config-warning-user-role'>console.warn('FMB Engine is disabled for current user role.');</script>\r\n";
			} );
...
function orderflow_round( $val, $precision = 2, $mode = PHP_ROUND_HALF_UP ) {
    if ( ! is_numeric( $val ) ) {
        $val = floatval( $val );
    }
...
function isEventEnabled( $eventName = '' ) {
	...
	foreach ( \fmb_engine\Facebook\FacebookRuntime()->getRegisteredPixels() as $pixel ) {
...
function getfmb_engineCurrencySymbols() {
    return array(
        'AED' => '&#x62f;.&#x625;',
        'AFN' => '&#x60b;',
        'ALL' => 'L',
...
function getStandardParams() {
    ...
    $params['event_day'] = $days[date('w', $current_time)];
    $params['event_hour'] = $hours[date('G', $current_time)];
    $params['event_month'] = $months[date('n', $current_time) - 1];

    
    $params['traffic_source'] = getTrafficSource();

    
    if ( isset( $_COOKIE['orderflow_landing_page'] ) && ! empty( $_COOKIE['orderflow_landing_page'] ) ) {
        $params['landing_page'] = $_COOKIE['orderflow_landing_page'];
    }

    if(\fmb_engine\Facebook\FacebookRuntime()->getOption("enable_user_role_param")) {
    ...
    if(\fmb_engine\Facebook\FacebookRuntime()->getOption("enable_event_url_param")) {
    ...
    if(is_singular( 'post' )) {
        ...
        $tags = getObjectTerms('post_tag', $post->ID);
        if (!empty($tags)) {
        ...
        $tags = getObjectTerms( 'product_tag', $post->ID );
        if ( ! empty( $tags ) ) {
            $params['tags'] = implode( ', ', $tags );
        }

        $categories = getObjectTerms( 'product_cat', $post->ID );
        if ( ! empty( $categories ) ) {
    ...
    if (empty($params['post_id']) || $params['post_id'] === "") {
        
        $queried_object_id = get_queried_object_id();
        if ($queried_object_id) {
            $params['post_id'] = $queried_object_id;
    ...
    if (empty($params['page_title']) || $params['page_title'] === "") {
        if (isset($post) && $post instanceof \WP_Post) {
            $params['page_title'] = $post->post_title;
        } else {
    ...
    if ( empty( $params['post_type'] ) ) {
        ...
        } elseif ( isWooCommerceActive() && function_exists( 'is_checkout' ) && is_checkout() ) {
            $params['post_type'] = 'page';
        } elseif ( isWooCommerceActive() && function_exists( 'is_cart' ) && is_cart() ) {
            $params['post_type'] = 'page';
        } elseif ( isWooCommerceActive() && function_exists( 'is_account_page' ) && is_account_page() ) {
            $params['post_type'] = 'page';
        } else {
            $params['post_type'] = 'page';
        }
...
function getTrafficSource () {
    ...
    try {
        ...
        $cookie = ! isset( $_COOKIE['orderflowTrafficSource'] ) ? null : $_COOKIE['orderflowTrafficSource'];
...
function getUtms ($seed_undefined = false) {
    ...
    foreach ($utmTerms as $utmTerm) {
        ...
        } elseif ( isset( $_COOKIE[ 'orderflow_' . $utmTerm ] ) ) {
            $utm[ $utmTerm ] = filterEmails( $_COOKIE[ 'orderflow_' . $utmTerm ] );
...
function get_persistence_user_data( $em, $fn, $ln, $tel ) {
	...
	if ( ! apply_filters( 'orderflow_disable_advanced_form_data_cookie', false ) && ! apply_filters( 'orderflow_disable_advance_data_cookie', false ) ) {
		$cookie_name = null;
		if ( isset( $_COOKIE['orderflow_advanced_form_data'] ) ) {
			$cookie_name = 'orderflow_advanced_form_data';
		}

		if ( $cookie_name ) {
			$userData = json_decode( stripslashes( $_COOKIE[ $cookie_name ] ), true );
			$data_persistence = FacebookRuntime()->getOption( 'data_persistency' );
...
function orderflow_get_option( $option, $default = false ) {
    global $wpdb;

    $table = $wpdb->prefix . 'orderflow_options';
...
function orderflow_update_option( $option, $value ) {
    global $wpdb;
